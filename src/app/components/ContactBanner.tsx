    ...
    >
      <div className="max-w-5xl mx-auto text-center">
        ...
        <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-8">
          <span className="text-white">{t('contact.subtitle_prefix')}</span>{' '}
          <span style={{ color: colors.orange }}>{t('contact.subtitle_suffix')}</span>
        </h2>
        ...
        <div className="flex flex-wrap justify-center gap-6 md:gap-10">
          {features.map((feature, idx) => (
            <div key={idx} className="flex items-center gap-2">
              {/* Tick icon - white circle with black checkmark */}
              <div className="w-5 h-5 bg-white rounded-full flex items-center justify-center flex-shrink-0">
                <svg className="w-3 h-3" fill={colors.black} viewBox="0 0 20 20">
